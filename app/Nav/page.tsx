...
function Nav() {
    ...
    function onScroll(){
        ...
        if(window.scrollY > lastMove && nav.current.style.transform == "translate(0px, -100%)"){
            change = 0;
        }else if(window.scrollY < lastMove && nav.current.style.transform == "translate(0px, 0px)"){
            change = 0;
    ...
    return ( 
        <nav className="nav" ref={nav}>
            <Link href="/" className="autono"><h3>AUTONO</h3></Link>
            ...
            <button ref={hamburger} className="burgerBtn" onClick={()=>{hamburger.current.classList.toggle("active")}}>
